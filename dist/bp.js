...
;(function($) {
    ...
    viewStyle = 'body {position: relative; margin: 0; padding: 0; width: 100%; overflow: hidden;}\
         * {margin: 0;padding: 0;}\
         ...
         html, body {background: #fff;width: 100%;font-family: Helvetica,sans-serif;-webkit-text-size-adjust: none;}\
         body {background: #fff;color: #999;font-size: 14px;font-family: arial;}\
         html, body {font-family: Helvetica,sans-serif;-webkit-text-size-adjust: none;}\
         input, img {vertical-align: middle;}\
        .bp-fullscreen {position: absolute; left: 0; top: 0; margin: 0; padding: 0; width: 100%; visibility: hidden; overflow: hidden; z-index: -1; }\
        .bp-page {position: absolute; left: 0; top: 0; bottom: 0; right: 0; margin: 0; padding: 0; overflow: hidden; z-index: 2000; -webkit-transform: translateZ(0); -webkit-backface-visibility: hidden; -webkit-transform-style: preserve-3d;}\
        .bp-page-bg {position: absolute; left: 0; top: 0; bottom: 0; right: 0; margin: 0; padding: 0; }\
        .bp-page-body {position: absolute; left: 0; top: 0; bottom: 0; right: 0; margin: 0; padding: 0; overflow: hidden; -webkit-transform: translateZ(0); -webkit-backface-visibility: hidden; -webkit-transform-style: preserve-3d;}\
        .bp-scroll {overflow: auto;}\
        .bp-scroll-touch {-webkit-overflow-scrolling: touch; }\
        .bp-scroll-x {overflow-y: hidden;}\
        .bp-scroll-y {overflow-x: hidden;}\
        .bp-cover {display: none; position: absolute; left: 0; right: 0; top: 0; bottom: 0; text-align: center; z-index: 5000; }\
        .bp-loader {display: none; position: absolute; left: 0; right: 0; top: 0; bottom: 0; text-align: center; overflow: hidden; z-index: 5001; }',
    // loading层元素
        loaderBody = '<div class="bp-loader-animate"><div class="bg"></div><span class="ball"></span><span class="ball"></span></div>',
    // loading层样式
        loaderStyle = '.bp-loader-animate {position: absolute; top: 50%; left: 50%;}\
        .bp-loader-animate .bg {position: absolute; width: 64px; height: 64px; margin: 0 auto; top: -32px; left: -32px; border-radius: 50%; background: #2C3E50; opacity: 0.5;}\
        .bp-loader-animate .ball {display: block; float: left; padding: 8px; margin-top: -8px; margin-left: -10px; -webkit-border-radius: 50%; -moz-border-radius: 50%; -ms-border-radius: 50%; -o-border-radius: 50%; border-radius: 50%;}\
        .bp-loader-animate span:nth-child(2) {background: #16A085; -webkit-animation: move-left 800ms ease-in-out infinite alternate; -moz-animation: move-left 800ms ease-in-out infinite alternate; -ms-animation: move-left 800ms ease-in-out infinite alternate; -animation: move-left 800ms ease-in-out infinite alternate;}\
        .bp-loader-animate .ball:nth-child(3) {background: #E67E22; -webkit-animation: move-right 800ms ease-in-out infinite alternate; -moz-animation: move-right 800ms ease-in-out infinite alternate; -ms-animation: move-right 800ms ease-in-out infinite alternate; animation: move-right 800ms ease-in-out infinite alternate;}\
    ...
    // 获取视图数据
    $.bp.getViewData = function($view) {
        return viewsdata[$view.data('id')]
    };

    // 获取当前视图
    $.bp.getCurPage = function() {
        return $curPage
    };
    ...
    ;(function() {
        ...
        $win.on('bp:adjustfullscreen' + ($.os.ios?' orientationchange':''), function(event) {
    ...
    $doc.on('bp:scroll', function(event, options) {
        var $target = $(event.target),
            direction = (options && options.direction) || '';

        $target.addClass('bp-scroll' + (direction ? ' bp-scroll-' + direction : ''));
    });

    $doc.on('bp:removescroll', function(event, options) {
        var $target = $(event.target);

        $target.removeClass('bp-scroll bp-scroll-x bp-scroll-y');
    });

    // ios设备才支持scrollfix
    $.os.ios && $doc.on('touchstart', '.bp-scroll, .bp-scroll-x, .bp-scroll-y', function(event) {
        ...
        if($target.hasClass('bp-scroll') || $target.hasClass('bp-scroll-x')) {
            if(scrollLeft < 0) {
        ...
        if($target.hasClass('bp-scroll') || $target.hasClass('bp-scroll-y')) {
            if(scrollTop < 0) {
                // event.preventDefault();
            }
            if(scrollTop <= 0) {
    ...
    //     $win.trigger('bp:adjustfullscreen')
    //   }
    // })
    ...
    $win.on('popstate', function(event) {
        ...
        if(!hashhistory.length || hashhistory[hashhistory.length - 1] !== hash) {
            ...
            if($page) {
                ...
                $page.trigger('bp:openpage');
            } else {
                $doc.trigger('bp:createpage', {hash: hash, pushData: pushData});
            }
        }
    });
    ...
    $doc.on('bp:route', function(event, options) {
        var args = Array.prototype.slice.call(arguments, 1);
        if(args.length > 1) {
            $.each(args, function(i, route) {
                $doc.trigger('bp:route', route)
            });
    ...
    //添加转场动画
    $doc.on('bp:addTransitPageAnimates', function(event, options) {
        var names = []
    ...
    $doc.trigger('bp:addTransitPageAnimates', {
        fadeIn: function($toPage, $fromPage, callback) {
            var $toPageBody = $('.bp-page-body', $toPage),
    ...
    //创建新页面
    $doc.on('bp:createpage', function(event, options) {
        $doc.trigger('bp:openloader')
        ...
        if(isRegExp(routeReg)) {
            var classname = (pageOptions.classname ? ' bp-page-' + pageOptions.classname : '') + (pageOptions.nocache ? ' no-cache' : ''),
                $page = $('<div class="bp-page' + classname + '"><div class="bp-page-body"></div></div>'),
                pageId = uniqueID(),
                pageData,
                viewData;
            ...
            // 缓存视图
            !pageOptions.nocache && $doc.trigger('bp:viewcache', {view: $page});
    ...
    //初始化页面
    $doc.on('bp:initpage', function(event, $page, viewData) {
        var pageId = $page.data('id'),
            pageData = viewsdata[pageId],
        ...
        // 关闭loader
        $doc.trigger('bp:closeloader');

        // 渲染视图内容
        $('.bp-page-body', $page).html(viewData.body);
    ...
    //打开页面
    $doc.on('bp:openpage', '.bp-page', function(event, afteropenCallback) {
        ...
        // 新建一个占位页面视图
        if(!$curPage) {
            $body.append('<div class="bp-page bp-page-empty"><div class="bp-page-body"></div></div>')
            $curPage = $('.bp-page-empty')
        }
        ...
        // $doc.trigger('bp:opencover')

        var callback = function() {
            if(!$page.data('bp:init')) {
                $page.data('bp:init', true)
                route.init.call($page, pageData)

                // 打开之前还原bp-scroll-touch
                if($.os.ios && parseInt($.os.version.slice(0, 1)) > 5) {
                    $('.bp-scroll', $page).addClass('bp-scroll-touch')
                }
            }

            // 关闭之后清除bp-scroll-touch
            if($.os.ios && parseInt($.os.version.slice(0, 1)) > 5) {
                $('.bp-scroll', $curPage).removeClass('bp-scroll-touch')
            }
        ...
        // 更改视图缓存顺序
        $doc.trigger('bp:viewcachesort', {view: $page})
    ...
    //定义面板
    $doc.on('bp:panel', function(event, options) {
        var args = Array.prototype.slice.call(arguments, 1)
        if(args.length > 1) {
            $.each(args, function(i, panel) {
                $doc.trigger('bp:panel', panel)
            })
    ...
    //创建新面板
    $doc.on('bp:createpanel', function(event, id, pushData) {
        var panelOptions = panels[id];

        if(panelOptions) {
            $doc.trigger('bp:openloader');

            var classname = panelOptions.classname ? ' bp-panel-' + panelOptions.classname : '',
                $panel = $('<div id="bp-panel-' + id + '" class="bp-page bp-panel ' + classname + '"><div class="bp-page-bg"></div><div class="bp-page-body"></div></div>'),
                panelData,
                viewData;
            ...
            //缓存面板
            $doc.trigger('bp:viewcache', {view: $panel});

            //获取视图数据
            viewData = panelOptions.view.call($panel, panelData);

            if($.isPlainObject(viewData)) {
                $panel.trigger('bp:initpanel', [$panel, viewData])
            }
        }
    });

    //初始化面板
    $doc.on('bp:initpanel', function(event, $panel, viewData) {
        ...
        // 渲染视图内容
        $('.bp-page-body', $panel).html(viewData.body);
        ...
        //触发打开页面事件
        $panel.trigger('bp:openpanel', [panelId, pushData])
    })

    //打开面板
    $doc.on('bp:openpanel', function(event, id, pushData) {
        //在页面加载和转场过程中阻止面板请求
        // if(($cover && $cover.css('display') === 'block') || ($loader && $loader.css('display') === 'block')) {
        //   return false
        // }
        ...
        if($panel) {
            ...
            if($curPage.hasClass('bp-panel')) {
                ...
                $prevPage.trigger('bp:openpage', [function() {
                    $doc.trigger('bp:openpanel', [id, pushData])
                }])
            ...
            var callback = function() {
                if(!$panel.data('bp:init')) {
                    $panel.data('bp:init', true)
                    panelOptions.init.call($panel, panelData)
                    ...
                    if($.os.ios && parseInt($.os.version.slice(0, 1)) > 5) {
                        ...
                        // 不如一帧以后再添加这个class
                        setTimeout(function() {
                            $('.bp-scroll', $panel).addClass('bp-scroll-touch')
                        }, 17)
            ...
            // 更改视图缓存顺序
            $doc.trigger('bp:viewcachesort', {view: $panel})

        } else {
            $doc.trigger('bp:createpanel', [id, pushData])
        }

    })

    //关闭面板
    $doc.on('bp:closepanel', function(event, options) {
        var $panel = $(event.target),
            panelId = $panel.data('id'),
            panelData = viewsdata[panelId];

        options && options.id && ($panel = $('.bp-panel-' + options.id))

        // 关闭面板后打开之前的页面
        if($curPage.hasClass('bp-panel') && $curPage.data('id') === panelId) {
    ...
    //点击面板外侧自动关闭面板
    $doc.on('click touchstart', '.bp-panel', function(event) {
        var $panel = $(event.currentTarget),
            $tagert = $(event.target);

        if($tagert.hasClass('bp-page-bg') || $tagert.hasClass('bp-panel')) {
    ...
    $doc.on('bp:viewcachecount', function(event, options) {
    ...
    $doc.on('bp:viewcache', function(event, options) {
    ...
    $doc.on('bp:viewcachesort', function(event, options) {
        var $view = options.view,
            viewId = $view.data('id'),
            type,
            key,
    ...
    $doc.on('bp:navigate', function(event, options) {
        var hash = options.hash || '',
            title = options.title || '',
            pushData = options.pushData || {},
        ...
        if(replace) {
            ...
            if(!$curPage.hasClass('bp-panel') && hashhistory.length && '#' + hashhistory[hashhistory.length - 1] === hash) {
                return
            }
    ...
    // 自定义loading层元素和样式
    $doc.on('bp:loader', function(event, options) {
    ...
    // 显示loading层
    $doc.on('bp:openloader', function(event) {
    ...
    // 隐藏loading层
    $doc.on('bp:closeloader', function(event) {
        if(loaderTimer) {
    ...
    $doc.on('bp:main', function(event, options) {
        // 初始化$body
        $body = $('body')

        // 注入样式
        $doc.trigger('bp:addstyle', viewStyle + loaderStyle)

        // 插入全屏模式监控层和loading层
        // $body.append('<div class="bp-fullscreen"></div><div class="bp-cover"></div><div class="bp-loader">' + loaderBody + '</div>')
        $body.append('<div class="bp-fullscreen"></div><div class="bp-loader">' + loaderBody + '</div>')

        // 调整全屏模式
        $fullscreen = $('.bp-fullscreen')
        $doc.trigger('bp:adjustfullscreen')
